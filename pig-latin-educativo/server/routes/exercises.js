 ...
 * Validate user's Pig Latin script
 */
router.post('/nivel/:nivel/ejercicio/:id/validar', async (req, res) => {
    try {
        ...
        let sanitizedScript;
        try {
            sanitizedScript = sanitizer.sanitizePigScript(script);
        } catch (sanitizeError) {
        ...
        const { stdout, stderr } = await dockerExecutor.executePigScript(
            sanitizedScript,
            `${nivel}_${id}`
        );
        ...
        if (exercise && exercise.expected_output_file) {
            ...
            try {
                ...
                // Pig writes actual errors with specific patterns
        ...
        const combinedOutput = stdout + '\n' + stderr;
        const suggestions = validator.analyzePigErrors(combinedOutput);
