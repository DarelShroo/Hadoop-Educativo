 ...
 * Validate user's Hive Latin script
 */
router.post('/nivel/:nivel/ejercicio/:id/validar', async (req, res) => {
    try {
        ...
        let sanitizedScript;
        try {
            sanitizedScript = sanitizer.sanitizeHiveScript(script);
        } catch (sanitizeError) {
        ...
        const { stdout, stderr } = await dockerExecutor.executeHiveScript(
            sanitizedScript,
            `${nivel}_${id}`
        );
        ...
        // Helper function to extract just data tuples
        function cleanDataOutput(output) {
        ...
        if (exercise && exercise.expected_output_file) {
            ...
            try {
                ...
                // Hive writes actual errors with specific patterns
                const combinedOutput = stdout + '\n' + stderr;
                const hasErrors = /\[main\] ERROR/i.test(combinedOutput) ||
                    /^ERROR /m.test(combinedOutput) ||
                    /FAILED.*Job/i.test(combinedOutput) ||
                    /Exception in thread/i.test(combinedOutput);
                const cleanedData = cleanDataOutput(stdout);
                const hasOutput = cleanedData.length > 0;

                comparisonResult = {
                    match: !hasErrors && hasOutput,
            ...
            const hasErrors = /\[main\] ERROR/i.test(combinedOutput) ||
                ...
                /FAILED.*Job/i.test(combinedOutput) ||
                /Exception in thread/i.test(combinedOutput);
            const hasOutput = cleanDataOutput(stdout).length > 0;
        ...
        const combinedOutput = stdout + '\n' + stderr;
        const suggestions = validator.analyzeHiveErrors(combinedOutput);
