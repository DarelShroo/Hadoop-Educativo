...
async function checkServerStatus() {
    try {
        ...
        if (status.docker && status.docker.running) {
...
function attachEventListeners() {
    // Level selection
    elements.nivelSelect.addEventListener('change', handleLevelChange);
...
function displayExerciseDetail(exercise) {
    ...
    elements.ejercicioDescripcion.textContent = exercise.description_es;
...
function loadProgress() {
    const saved = localStorage.getItem('pigLatinProgress');
    return saved ? JSON.parse(saved) : {};
}

function saveProgress() {
    localStorage.setItem('pigLatinProgress', JSON.stringify(state.progress));
}
