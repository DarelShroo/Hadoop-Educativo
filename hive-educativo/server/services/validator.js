...
/**
 * Extracts actual data output from Pig's verbose output
 * @param {string} pigOutput 
 * @returns {string}
 */
function extractDataOutput(pigOutput) {
    if (!pigOutput) return '';

    const lines = pigOutput.split('\n');
    const dataLines = [];
 ...
 * @param {string} userOutput - Raw output from user's Pig script
...
function compareOutputs(userOutput, expectedOutput, comparisonType = 'fuzzy') {
    // Extract actual data from Pig's verbose output
 ...
 * @param {string} stderr - Error output from Pig
 * @returns {string[]} Array of suggestions
 */
function analyzePigErrors(stderr) {
    const suggestions = [];
    ...
    if (suggestions.length === 0 && stderr.length > 0) {
        suggestions.push('Revisa el mensaje de error completo. Los errores de Pig suelen indicar la línea problemática.');
    }
